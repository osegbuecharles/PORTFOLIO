...
$(document).ready(async function(){    
    ...
    $(dataId).ready(function(){
        ...
        $("#blogShareFooter").html(`
            <a target="_blank" onclick="share(`+data.id+`)" href="https://twitter.com/intent/tweet?text=`+data.subject+`&nbsp;by&nbsp;`+data.author+`&url=`+shareurl+`%2F?id=`+data.id+`%2F&via=AUIAA" title="Share on Twitter"><i style="font-size:x-large;" class="fa fa-twitter text-primary"></i></a>
            <a target="_blank" onclick="share(`+data.id+`)" href="https://web.facebook.com/sharer/sharer.php?u=`+shareurl+`?id=`+data.id+`" title="Share on Facebook"><i style="font-size:x-large;" class="fa fa-facebook text-primary"></i></a>
...
async function loadBlogInfo(){
    url=getAllUrlParams();
    $("#blogComments").html(`<img src="${preloader}" style="width:20%;" />`);
